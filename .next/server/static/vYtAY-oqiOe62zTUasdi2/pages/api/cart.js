...
async function handlePutRequest(req, res) {
  ...
  try {
    const {
      userId
    } = jsonwebtoken__WEBPACK_IMPORTED_MODULE_1___default.a.verify(req.headers.authorization, "hfkskjdkjdkd"); // Get user cart based on userId

    const cart = await _models_Cart__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"].findOne({
      user: userId
    }); // Check if product already exists in cart

    const productExists = cart.products.some(doc => ObjectId(productId).equals(doc.product)); // if so, increment quantity on product ID
